...
\author{Geunsik~Lim%
\thanks{Independent researcher. Corresponding author: leemgs@gmail.com.}%
}

\markboth{IEEE Transactions on Computers}%
{Lim: Truthprint: Designing an Invariant-Constrained Semantic IR for Provenance Watermarking}

\begin{document}
\maketitle

\begin{abstract}
Token-level watermarks for large language models (LLMs) are fragile under translation and paraphrasing because their signal lives in surface token choices. This paper proposes \emph{Truthprint}, a semantic intermediate representation (IR) that separates \emph{locked invariants} (polarity, quantity, time, attribution, roles) from \emph{controlled realization variables} through a machine-checkable, typed mutability contract. We evaluate two provenance mechanisms that share this IR and are careful to distinguish them. (i)~A \emph{realization-carrier} scheme embeds an authenticated, error-corrected payload in realization freedom; against real neural machine translation this is an \emph{honest negative}---the Stage-1 surface parser recovers $0/192$ NLLB outputs---because translation rewrites the carriers. (ii)~A \emph{meaning-digest} scheme instead registers a keyed tag over the invariant digest in an out-of-band ledger and re-verifies it after transformation, carrying no realization payload. On $4{,}800$ real NLLB translations across six languages, a Stage-2 lexicon-based extractor recovers invariant fields at $0.90$--$1.00$ and the meaning-digest scheme authenticates translated documents cross-lingually ($1.000$ document-level attribution for five of six languages; Arabic weaker) while rejecting single-field tampering. We prove three \emph{conditional} properties (protected-field fidelity under a sound validator, fresh-payload unforgeability reducing to the MAC, and a per-key--nonce tag-acceptance bound $2^{-\tau}$), and ship a seeded, self-testing cryptographic/coding core (zero authenticated false positives in $20{,}000$ trials; $95\%$ upper bound $1.5\!\times\!10^{-4}$). All results are closed-domain and small-scale; wide-coverage neural parsing, semantic-level false-positive analysis, official baselines, and adaptive attacks remain necessary before any comparative robustness claim. Truthprint does not identify arbitrary AI text; it verifies consistency with a participating keyed generator.
\end{abstract}
...
Regulation also motivates work on machine-readable marking. Article~50 of the EU Artificial Intelligence Act (Regulation (EU) 2024/1689) introduces transparency obligations concerning synthetic outputs, including text \cite{euaiact2024}. Truthprint is a research design, however, and we neither interpret the regulation nor claim that the proposed mechanism satisfies it; robustness to meaning-preserving transformation is only a technical objective here \cite{liu2024survey}. Reliable provenance also matters for data hygiene: training on unattributed synthetic text can degrade model quality over successive generations, a failure mode documented at scale in \emph{Nature} \cite{shumailov2024collapse}. Truthprint targets this gap with an authenticated design whose cross-lingual robustness remains to be established empirically.
...
We can state the delta over the closest prior work precisely. Embedding-based semantic watermarks \cite{hou2024semstamp,ren2024semmark,liu2023sir,schaefer2026dew} define admissibility through a \emph{metric}: two texts are treated as equivalent when their representations are close under a learned distance. Truthprint instead defines admissibility through a \emph{typed predicate}: a transformation is admissible only when a symbolic invariant relation holds field by field. A watermark symbol then ranges not over tokens or embedding regions but over the key-dependent equivalence classes induced by that relation. This has two consequences that motivate the analysis in Section~\ref{sec:analysis}. First, conditional on sound extraction and validation, encoding only moves within an invariant-equivalence class and therefore preserves the protected fields. Second, because authenticated attribution is gated by a payload bound to the invariant digest rather than by a similarity score, fresh-payload forgery reduces to the security of the message-authentication code. This statement does not cover the fallback statistical detector, whose system-level false-positive rate still requires held-out calibration. Structured-semantics watermarking such as SWAN \cite{ye2026swan} shares the use of a graph carrier but adopts an existing meaning formalism wholesale; Truthprint instead types each field with an explicit mutability policy and validates every realization against the resulting contract before release.

\subsection{Contributions}
This work makes the following contributions:
\begin{itemize}[leftmargin=*]
  \item \textbf{Method.} We formulate watermark carriers as secret-keyed realization choices constrained by a field-level mutability contract (\emph{locked}, \emph{conditional}, or \emph{realization-only}), and combine uncertain-carrier erasures with error correction and invariant-bound payload authentication.
  \item \textbf{Conditional analysis.} Assuming sound invariant extraction and validation, we establish protected-field fidelity; under standard cryptographic assumptions, we relate fresh-payload forgery and per-test tag acceptance to the message-authentication code. We also state the finite-code rank condition rather than presenting the observed erasure transition as a universal threshold.
  \item \textbf{Stage-1 evidence and falsifiable protocol.} A self-contained implementation verifies the coding and cryptographic core, while a closed-domain English realization demonstrates composition on sentence strings. We explicitly separate these measured results from a diagnostic simulation and specify the multilingual, official-baseline evaluation needed to test---not presume---translation and paraphrase robustness.
\end{itemize}

\subsection{Claim--Evidence Boundary}
To prevent the design goal from being mistaken for an empirical result,
Table~\ref{tab:claims} separates what the present artifact demonstrates, what
the analysis proves only under explicit assumptions, and what remains a
hypothesis. In particular, ``language-neutral'' describes the intended schema
interface rather than a measured property of the current English template
frontend.

\begin{table*}[t]
\centering
\caption{Claim--evidence map for the present submission. ``Not established''
means that the claim must not be inferred from the diagnostic simulation.}
\label{tab:claims}
\begin{tabular}{p{3.5cm}p{3.2cm}p{8.5cm}}
\toprule
Claim & Status & Supporting evidence or missing test \\
\midrule
Coding/authentication round trip & Demonstrated & Deterministic core tests P1--P4 and closed-domain string tests L1--L3. \\
...
Meaning-layer recovery under real MT & Demonstrated (closed-domain, six languages) & Stage-2 extractor recovers fields at $0.90$--$1.00$ over $4{,}800$ real NLLB translations in six languages, document attribution $1.000$ (CI $[0.929,1.000]$) for five of six languages, and meaning-based authentication attributes cross-lingually (Secs.~\ref{sec:stage2},~\ref{sec:provenance}); wide-coverage parsing and larger document counts remain open. \\
Paraphrase/adaptive-attack robustness & Not established & Requires meaning-preserving attack outputs, held-out detector calibration, and ValidRemoval evaluation. \\
...
Text watermarking is relevant to emerging transparency regulation, but no watermark design alone establishes legal compliance. Under Article~50 of the EU AI Act, providers of generative systems must mark synthetic outputs in a machine-readable, detectable format \cite{euaiact2024}; the accompanying policy discussion stresses that a mark is only useful if it is \emph{robust}, since a signal that dissolves under paraphrasing or translation cannot support reliable downstream attribution \cite{liu2024survey}. This motivates the technical problem addressed here without treating robustness as a legal conclusion. Truthprint is proposed as a possible semantic layer complementary to token-level marks, not as a compliance product.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Controlled authenticated false positives:} under the stated PRF and independence assumptions, an independently produced decoded payload rarely passes the keyed tag check; the statistical fallback requires separate empirical calibration.
\end{enumerate}
...
Assumption~\ref{as:validator} is load-bearing, so we measure its empirical
proxy---the validator's \emph{false-accept rate} on meaning-altering edits---
rather than assume it silently. A false accept is a single-field tamper that the
re-extracted invariants fail to flag. On the closed domain the typed contract
detects every single-field tamper across all six protected fields (property C1,
aggregate $1.000$, CI $[0.998,1.000]$: zero observed false accepts) while
retaining attribution across benign paraphrases ($0.990$); the decode-only
ablation accepts every tamper (property C2, $0.000$), confirming that the
invariant re-extraction---not the coding layer---is what supplies soundness.
Under real NLLB translation the Stage-2 extractor still rejects meaning-altering
edits at $\ge 0.99$ per condition (Sec.~\ref{sec:provenance}). These are
empirical soundness measurements on the closed and machine-translated domains; a
\emph{wide-coverage} validator's soundness remains open and is what the neural
frontend of Appendix~\ref{app:roadmap} must establish, so Proposition~\ref{prop:fidelity}
stays conditional rather than absolute.

\subsection{Unforgeability and False Positives}
\begin{theorem}[Fresh-payload unforgeability]
\label{thm:unforge}
...
\begin{theorem}[Authenticated acceptance under an ideal-PRF model]
\label{thm:fpr}
...
\end{theorem}
This statement concerns only a candidate that reaches tag verification. For $q$ distinct fresh candidates whose proposed tags are independent of ideal-PRF outputs, the union bound is $q2^{-\tau}$; adaptivity or repeated inputs must be covered by the concrete security game rather than assumed away. It is not an end-to-end detector FPR bound: parsing, candidate search, replay, carrier dependence, threshold calibration, and the number of tested keys and nonces all affect system-level error. Replayed generator outputs are also not fresh forgeries and require a nonce registry or provenance ledger.
The per-test bound is a design constant, not a tuned threshold: at $\tau=32$ it is $2.3\times10^{-10}$. The $0/20000$ observation in Section~\ref{sec:impl} is consistent with that bound but does not estimate such a small probability; its approximate 95\% binomial upper bound is $1.5\times10^{-4}$. When full payload recovery fails, the fallback statistic of Eq.~\eqref{eq:zscore} is used. Under the null hypothesis of no watermark, the keyed match count over $n$ reliable independent binary carriers is $k\sim\mathrm{Binomial}(n,\tfrac12)$, so by Hoeffding's inequality $\Pr[z\ge t\mid H_0]\le e^{-t^2/2}$, giving a calibrated operating point. Dependence among carriers is an idealization that production calibration must correct (Sec.~\ref{sec:detection}).
...
\end{proposition}
Equation~(\ref{eq:cliff}) yields a necessary carrier-count condition
$N\geq\lceil(\mu+\tau)/((1-\epsilon)\log_2\bar q)\rceil$ for average carrier
arity $\bar q$, together with the code-design constraint $R\leq1-\epsilon$.
This is a planning bound, not a guarantee for a particular finite code.  It
explains why short or heavily summarized text can fall below capacity; the
transition in Table~\ref{tab:erasure} is specific to the implemented code and
seeded erasure experiment.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{P3 (cryptographic false positives).} Over $20{,}000$ independent documents, $0$ were attributed. This is consistent with Theorem~\ref{thm:fpr}; statistically, the observation alone only gives an approximate 95\% binomial upper bound of $1.5\times10^{-4}$.
...
\subsection{Real-MT Pilot: An Honest Negative Result}
\label{sec:realmt}
To test the closed-domain frontend against \emph{real} machine translation
rather than a channel model, we ran a small end-to-end pilot: $4$ documents of
$16$ templated sentences were translated with a real neural MT system
(NLLB-200-distilled-600M) into Korean and Hindi and back-translated
(round-trip), yielding $192$ real translation outputs. Measuring objectively---%
by attempting to re-parse each translated sentence with the Stage-1 frontend,
so that a carrier counts as recovered only if the frontend can actually read
it---the frontend parsed $0/192$ outputs (parse coverage $0.000$, $95\%$ CI
$[0.000,0.020]$), including the round-trip English, which the rigid template
parser cannot read once a neural system rephrases it. Consequently no payload
was recovered from real MT. This is a deliberately honest negative result: it
confirms with real translations what Table~\ref{tab:claims} lists as ``not
established,'' and it localizes the bottleneck precisely at wide-coverage
semantic parsing rather than at the coding or cryptographic core (which
Sections~\ref{sec:impl} and~\ref{sec:challenge} validate). It also shows why a
naive evaluation must measure carrier survival from the transformed text
itself: an annotation that merely \emph{asserts} carrier survival can report a
spurious perfect score. The pilot harness and the objective measurement ship
with the artifact.

\subsection{Stage-2 Multilingual Invariant Extractor}
\label{sec:stage2}
The pilot's negative result is a property of the rigid Stage-1 \emph{surface}
parser, not of the meaning layer. To test whether the typed invariants
\emph{themselves} survive real translation, we add a closed-domain, lexicon-based
multilingual extractor (English, Korean, Hindi, Chinese, Arabic, and German)
that reads a translated sentence and recovers the nine invariant fields
directly. We evaluate it on $4{,}800$ real NLLB translations spanning all six
languages---English round-trip and direct translation into Korean, Hindi,
Chinese, Arabic, and German ($800$ source sentences over $50$ documents $\times$
six conditions). Where the Stage-1 parser recovered $0/192$, the Stage-2
extractor recovers the meaning layer at high rates (aggregate over all
conditions, Wilson $95\%$ CI): temporal direction $0.997$ $[0.995,0.998]$,
agent $0.997$, polarity $0.992$, causation $0.975$, quantity $0.974$,
attribution $0.969$, predicate $0.960$, modality $0.910$, and patient $0.905$
$[0.896,0.913]$. Recovery is strong for Hindi, Korean, Chinese, and German (per-%
condition all-exact recovery $0.81$--$0.88$) and weakest for Arabic ($0.29$),
where the MT system renders entities most variably (Arabic patient recovery
$0.50$). Two residual error sources are honest and instructive: entities the MT
system renders inconsistently (e.g.\ ``config drift'', ``cache miss'', and
Arabic patient nouns), and round-trip translation that itself alters a field
(e.g.\ ``must not''$\to$``may not''), which the extractor faithfully reports
rather than masks. This is direct real-data
evidence for the central hypothesis: meaning-layer fields survive machine
translation and are machine-recoverable, whereas the surface carriers of
Section~\ref{sec:realmt} do not. Because the recovered fields determine the
invariant digest, a meaning-altering edit to any recovered field is detected
across languages. It remains a closed-domain result---a wide-coverage neural
semantic parser is the next step---but it moves multilingual invariant recovery
from ``not established'' to a measured, positive finding on real translations.
The extractor and this evaluation ship with the artifact
(\texttt{scripts/eval\_multilingual.py}).

\subsection{Meaning-Based Provenance Authentication Survives Real Translation}
\label{sec:provenance}
Recovering the meaning layer lets us realize the paper's actual objective---%
\emph{verifying consistency with a participating keyed generator}---without a
surface payload that translation would erase. The producer registers, per
sentence, a keyed $\tau$-bit tag over the canonical digest of a declared
\emph{invariant contract} (a chosen field subset), stored in the out-of-band
provenance ledger (like the nonce). The verifier re-extracts the invariants from
the transformed sentence (Stage-2) and authenticates iff the tag reproduces.
Authentication thus succeeds exactly when the contract's meaning is preserved
and recovered; a meaning-altering edit changes a contract field and is rejected;
an unrelated document matches only with probability $2^{-\tau}$. On $4{,}800$ real
NLLB translations across six languages (Wilson $95\%$ CI, $\tau=32$), a compact
contract (polarity, quantity, temporal direction, attribution, predicate, agent)
yields sentence-level authentication TPR of $0.968$ (DE), $0.968$ (KO), $0.955$
(ZH), $0.946$ (HI), $0.936$ (round-trip), and $0.629$ (AR); tamper rejection
$\ge 0.99$; and false positives consistent with the cryptographic bound
($0.001$--$0.010$). Aggregating to the document level (a document is attributed
when at least half of its sentences authenticate) gives $1.000$ attribution
($95\%$ CI $[0.929,1.000]$) for five of six conditions and $0.920$ for Arabic,
over $50$ documents.
Widening the contract to all nine fields lowers TPR (e.g.\ $0.289$ for AR) but authenticates more meaning:
an explicit, measured fidelity/robustness trade-off. This is the translation-%
robust counterpart to the surface-carrier pilot's $0/192$: provenance verified
on the meaning layer survives real machine translation. One caveat is essential
and we make it quantitative. Because authentication reproduces a tag over an
invariant \emph{contract}, two genuinely distinct but low-entropy generations
that share the same compact contract collide \emph{before} the tag is tested:
their contract digests are equal, so the tag matches deterministically. The
achievable false-positive rate is therefore bounded below by the
\emph{contract-collision probability}---governed by the entropy of the chosen
contract on the deployment text---not by $\tau$. Measuring this floor on the
closed-domain corpus (\texttt{scripts/eval\_semantic\_fp.py}) is revealing: the
six-field contract admits only $192$ distinct values ($7.6$ bits), giving a
collision floor of $5.2\times10^{-3}$ ($\approx1/192$). This \emph{matches} the
$0.001$--$0.010$ false positives measured above and is $\sim\!2\times10^{7}$
times the cryptographic $2^{-32}$ bound. In other words, the meaning-digest
scheme's observed false positives are the contract-entropy floor, not the tag
bound; widening to nine fields lowers the floor to $\sim\!1.8\times10^{-4}$ at
the cost of translation robustness. Open-domain and especially short or
formulaic text has \emph{lower} contract entropy and thus a \emph{higher} floor,
so estimating contract entropy per corpus is a deployment prerequisite; the
$2^{-\tau}$ bound applies only once contract entropy exceeds $\tau$ bits.
It remains a closed-%
domain Stage-2 result; a wide-coverage semantic parser and more documents/%
languages are the next steps. The method and evaluation ship with the artifact
(\texttt{scripts/eval\_provenance.py}).

To respect the page budget, three supporting parts are placed in the
appendices: full reproducibility and notation (Appendix~\ref{app:repro}),
the reference software architecture and staged prototype roadmap
(Appendices~\ref{app:arch}--\ref{app:roadmap}), and the extended evaluation
protocol---baselines, languages, attacks, and metrics
(Appendix~\ref{app:protocol}). The body retains the research questions and
the decisive evaluation matrix.
...
\bottomrule
\multicolumn{4}{p{0.92\columnwidth}}{\footnotesize $^\dagger$Controlled
