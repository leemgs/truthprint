...
Semantic-level false-positive rate (contract collision) & Not established & The $2^{-\tau}$ bound covers cryptographic tag collision only. Distinct legitimate generations that share a low-entropy invariant contract collide \emph{before} the tag, so short or formulaic text requires a measured contract-entropy / semantic-collision analysis not yet performed (Sec.~\ref{sec:provenance}). \\
\bottomrule
\end{tabular}
\end{table*}
...
the reported false positives ($0.001$--$0.010$) reflect the \emph{cryptographic}
$2^{-\tau}$ tag collision, not the \emph{semantic} collision rate. Because
authentication reproduces a tag over an invariant contract, two genuinely
distinct but low-entropy generations that share the same compact contract (a
short or formulaic factual sentence) would collide \emph{before} the tag is
tested; the achievable semantic-level false-positive rate is therefore bounded
by the entropy of the chosen contract, not by $\tau$. Measuring contract entropy
and the induced semantic-collision rate on open-domain text is an open item
(Table~\ref{tab:claims}) and a prerequisite for any deployment on short text.
It remains a closed-%
