...
- **노트북 STEP 4에서 `TooManyRequests`(429)** 가 계속 뜨면(특히 Kaggle) 데이터센터 IP가
  Google 무료 엔드포인트에서 차단된 것입니다. STEP 4는 이미 **MyMemory→Google** 순으로
  폴백하지만, 둘 다 막히면 **STEP 4B(로컬 NLLB)** 셀을 실행하세요 — 모델을 내려받아
  오프라인으로 번역하므로 IP 차단과 무관하게 동작합니다.
