...
- **노트북 STEP 4에서 `TooManyRequests`(429)** 가 뜨면 Google 번역 속도제한입니다. 셀에
  이미 재시도·지연이 들어 있어 대개 자동 복구되며, 계속 실패하면 셀 안의 `THROTTLE` 값을
  `2.0`~`3.0`으로 올리고 그 셀만 다시 실행하세요(실패한 문장만 이어서 채워집니다).
